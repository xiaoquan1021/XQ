...
function Initialize-XQEnvironment {
    param(
        ...
        [string]$Build,
        [string]$ExternalRoot,
        [string]$ExternalPlatform = "windows-x64-blueberry"
    )
    ...
    $xqPluginPath = Join-Path $buildRoot "lib\plugins"
    $blueBerryPluginCandidates = @(
        $xqPluginPath,
        (Join-Path $buildRoot "bin\plugins"),
        (Join-Path $resolvedExternals (Join-Path "build" (Join-Path $ExternalPlatform "MITK\MITK-build\bin\plugins"))),
        (Join-Path $resolvedExternals (Join-Path "build" (Join-Path $ExternalPlatform "MITK\MITK-build\lib\plugins"))),
        (Join-Path $installRoot "mitk-2024.06\bin\plugins"),
        (Join-Path $installRoot "mitk-2024.06\lib\plugins"),
        (Join-Path $installRoot "mitk-2024.06\plugins")
    )
    $blueBerryPluginPaths = $blueBerryPluginCandidates |
        Where-Object { $_ } |
        Select-Object -Unique

    $env:XQ_PLUGIN_PATH = $xqPluginPath
    $env:BLUEBERRY_PLUGIN_PATH = ($blueBerryPluginPaths -join ";")
    ...
    [pscustomobject]@{
        ...
        BlueBerryPluginPath = $env:BLUEBERRY_PLUGIN_PATH
    }
}
