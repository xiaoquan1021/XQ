...
int main()
{
    ...
    {
        // B6c: threshold -> keepLargestConnectedComponent is the two-step the
        // controller's threshold path now runs. A vessel threshold selects every
        // in-band voxel volume-wide, so the raw mask keeps scattered tissue
        // speckle; the largest-component step must drop everything but the vessel
        // block. Synthetic line (histogram fully known): a large in-band block of
        // 3 (voxels 0,1,2), a background gap (voxel 3), then a lone in-band voxel
        // (4). threshold [128,255] selects all 4; the largest component is the
        // 3-voxel block, so voxel 4 must be cleared. Skipping the largest-
        // component step leaves voxel 4 set and reddens the checks below.
        SyntheticImage syn = make_uint8_image(6, 1, 1, {200, 200, 200, 0, 200, 0});
        CHECK(syn.buffer->is_valid());
        ...
        const xq::SegmentationService::Result thresholded =
            xq::SegmentationService::thresholdMask(syn.image, *syn.buffer, params, image_id);
        CHECK(thresholded.ok());
        CHECK(thresholded.mask != nullptr);
        // raw threshold keeps both blobs (4 voxels, volume-wide, no connectivity)
        CHECK(thresholded.mask->foregroundVoxelCount() == 4);

        const xq::SegmentationService::Result largest =
            xq::SegmentationService::keepLargestConnectedComponent(*thresholded.mask);
        CHECK(largest.ok());
        CHECK(largest.mask != nullptr);
        // only the 3-voxel block survives; the lone voxel is dropped
        CHECK(largest.mask->foregroundVoxelCount() == 3);
        CHECK(largest.mask->labelAt(0) == 1);
        CHECK(largest.mask->labelAt(1) == 1);
        CHECK(largest.mask->labelAt(2) == 1);
        CHECK(largest.mask->labelAt(4) == 0);
        // provenance survives the largest-component step
        CHECK(largest.mask->hasSourceImageNode());
