...
int test_future_minor_rejected()
{
    ...
    if (status != xq::XQProjectReader::Status::UnsupportedVersion) {
        return fail("future minor version is rejected", __LINE__);
    }
    if (result.diagnostics.empty()
        || result.diagnostics[0].code() != "PROJECT_UNSUPPORTED_SCHEMA_VERSION") {
        return fail("future minor rejection is structured", __LINE__);
    }
    return 0;
}
...
int test_v12_node_fields_migrate_to_absent_and_zero()
{
    const std::filesystem::path path = temp_project_path("xq_shell_v12_node_migration.xqproj");
    const std::string text =
        "XQ_NATIVE_PROJECT schemaVersion 1.2\n"
        "writerVersion XQ-M9-001\n"
        "minimumReaderVersion 1.2\n"
        "createdWith XQrebuild\n"
        "projectId v12-node-migration\n"
        "scene\n"
        "nodes 1\n"
        "node 4201 image LegacyImage\n"
        "relations 0\n"
        "stale 0\n"
        "endScene\n"
        "assets 0\n"
    ...
    std::filesystem::remove(path);
    if (!write_text_file(path, text)) {
        return fail("write schema 1.2 migration fixture", __LINE__);
    }
    xq::XQProjectReadResult result = {};
    const xq::XQProjectReader::Status status =
        xq::XQProjectReader::load(path.string(), &result);
    std::filesystem::remove(path);
    if (status != xq::XQProjectReader::Status::Ok || !result.diagnostics.empty()) {
        return fail("schema 1.2 migration fixture loads", __LINE__);
    }
    const xq::XQDataNode* node = result.project.scene().find(xq::NodeId(4201));
    if (node == nullptr || node->hasScaleSlot() || node->contentRevision() != 0) {
        return fail("schema 1.2 node migrates to unspecified scale and revision zero", __LINE__);
    }
    return 0;
}
...
int test_current_v11_roundtrip_preserves_node_ids()
{
    ...
    xq::XQDataNode* scaled = project.scene().find(xq::NodeId(5102));
    if (scaled == nullptr) {
        return fail("find node for schema 1.3 fields", __LINE__);
    }
    scaled->setScaleSlot(xq::ScaleSlot::Micro);
    scaled->setContentRevision(7);
    ...
    if (saved_text.find("XQ_NATIVE_PROJECT schemaVersion 1.3\n") == std::string::npos) {
        std::filesystem::remove(path);
        return fail("writer emits schemaVersion 1.3", __LINE__);
    }
    if (saved_text.find("node 5102 image.volume Volume%20Baseline scale micro revision 7\n")
        == std::string::npos) {
        std::filesystem::remove(path);
        return fail("writer emits fixed scale/revision node fields", __LINE__);
    }
    if (saved_text.find("\nprovenance\n") == std::string::npos) {
        std::filesystem::remove(path);
    ...
    const xq::XQDataNode* loadedScaled = result.project.scene().find(xq::NodeId(5102));
    if (loadedScaled == nullptr || !loadedScaled->hasScaleSlot()
        || loadedScaled->scaleSlot().value() != xq::ScaleSlot::Micro
        || loadedScaled->contentRevision() != 7) {
        return fail("schema 1.3 restores scale and revision", __LINE__);
    }
...
int main()
{
    ...
    result = test_future_minor_rejected();
    if (result != 0) {
        return result;
    }
    ...
    result = test_v12_node_fields_migrate_to_absent_and_zero();
    if (result != 0) {
        return result;
    }
