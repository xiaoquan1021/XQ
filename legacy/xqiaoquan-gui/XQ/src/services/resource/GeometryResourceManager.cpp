...
constexpr int kResidentVoxelInstalled = 2;
...
bool residentVoxelBytes(const VoxelMeta& meta, std::size_t* outBytes)
{
    if (outBytes == nullptr || !meta.valid || meta.type == ScalarType::Unknown
        || meta.components <= 0 || meta.dims[0] <= 0
        || meta.dims[1] <= 0 || meta.dims[2] <= 0) {
        return false;
    }
    ...
    std::size_t bytes = 0;
    const std::size_t scalarBytes = XQMemoryImageBufferHandle::scalarSize(meta.type);
    if (scalarBytes == 0
        || !checkedMul(static_cast<std::size_t>(meta.dims[0]),
                       static_cast<std::size_t>(meta.dims[1]), &xy)
        || !checkedMul(xy, static_cast<std::size_t>(meta.dims[2]), &voxels)
        || voxels != meta.voxelCount
        || !checkedMul(voxels, static_cast<std::size_t>(meta.components), &scalars)
        || !checkedMul(scalars, scalarBytes, &bytes)) {
        return false;
    }
    *outBytes = bytes;
    return true;
...
bool sameResidentAssetAssociation(const AssetRecord& left,
                                  const AssetRecord& right)
{
    if (left.id != right.id
        || left.category != right.category
        || left.kind != right.kind
        || left.sourceAbsPath != right.sourceAbsPath
        || left.sourceRelPath != right.sourceRelPath
        || left.hasDicom != right.hasDicom
        || left.contentFingerprint != right.contentFingerprint
        || left.hasGeometry != right.hasGeometry) {
        return false;
    }
    if (left.hasDicom
        && (left.dicom.studyInstanceUid != right.dicom.studyInstanceUid
            || left.dicom.seriesInstanceUid != right.dicom.seriesInstanceUid
            || left.dicom.frameOfReferenceUid
                != right.dicom.frameOfReferenceUid)) {
        return false;
    }
    if (!left.hasGeometry) {
        return true;
    }
    if (left.geometry.coordinateSystem != right.geometry.coordinateSystem) {
        return false;
    }
    for (int axis = 0; axis < 3; ++axis) {
        if (left.geometry.dimensions[axis]
                != right.geometry.dimensions[axis]
            || left.geometry.spacing[axis] != right.geometry.spacing[axis]
            || left.geometry.origin[axis] != right.geometry.origin[axis]) {
            return false;
        }
        for (int component = 0; component < 3; ++component) {
            if (left.geometry.direction[axis][component]
                != right.geometry.direction[axis][component]) {
                return false;
            }
        }
    }
...
GeometryResourceManager::ResidentKey
GeometryResourceManager::residentVoxelBlockKey(const AssetId& id)
{
    return ResidentKey(id, kResidentVoxelInstalled);
}
...
bool GeometryResourceManager::usesAssetRegistry(
...
GeometryResourceManager::installResidentVoxelSource(
    const AssetId& id,
    std::shared_ptr<const IVoxelSource> source)
{
    std::lock_guard<std::mutex> lock(mutex_);
    const AssetRecord* registeredAsset =
        registry_ == nullptr ? nullptr : registry_->find(id);
    if (!id.is_valid() || source == nullptr || registeredAsset == nullptr) {
        return VoxelSourceHandle();
    }

    std::size_t bytes = 0;
    if (!residentVoxelBytes(source->meta(), &bytes)) {
        return VoxelSourceHandle();
    }

    const ResidentKey key = residentVoxelBlockKey(id);
    auto existing = blocks_.find(key);
    if (existing != blocks_.end()) {
        if (existing->second.assetSnapshot == nullptr
            || !sameResidentAssetAssociation(
                *existing->second.assetSnapshot, *registeredAsset)) {
            residentBytes_ -= existing->second.bytes;
            lru_.erase(existing->second.lruIt);
            blocks_.erase(existing);
        } else {
            if (existing->second.voxelSource != source) {
                return VoxelSourceHandle();
            }
            ++hitCount_;
            lru_.erase(existing->second.lruIt);
            lru_.push_front(key);
            existing->second.lruIt = lru_.begin();
            return VoxelSourceHandle(existing->second.voxelSource,
                                     existing->second.pin);
        }
    }

    std::size_t nextResidentBytes = 0;
    if (!checkedAdd(residentBytes_, bytes, &nextResidentBytes)) {
        return VoxelSourceHandle();
    }

    ResidentBlock block;
    block.voxelSource = std::move(source);
    block.assetSnapshot = std::make_shared<AssetRecord>(*registeredAsset);
    ...
    std::shared_ptr<const IVoxelSource> sourcePtr =
        inserted.first->second.voxelSource;
    std::shared_ptr<void> pin = inserted.first->second.pin;
    evictToBudgetLocked();
    return VoxelSourceHandle(std::move(sourcePtr), std::move(pin));
}

GeometryResourceManager::VoxelSourceHandle
GeometryResourceManager::acquireResidentVoxelSource(const AssetId& id)
{
    std::lock_guard<std::mutex> lock(mutex_);
    const ResidentKey key = residentVoxelBlockKey(id);
    ...
    const AssetRecord* registeredAsset =
        registry_ == nullptr ? nullptr : registry_->find(id);
    if (registeredAsset == nullptr || it->second.assetSnapshot == nullptr
        || !sameResidentAssetAssociation(
            *it->second.assetSnapshot, *registeredAsset)) {
...
bool GeometryResourceManager::removeResidentVoxelSource(const AssetId& id)
{
    std::lock_guard<std::mutex> lock(mutex_);
    const ResidentKey key = residentVoxelBlockKey(id);
    auto it = blocks_.find(key);
...
bool GeometryResourceManager::removeResidentVoxelSourceIfMatches(
    const AssetId& id,
    const VoxelSourceHandle& expected)
{
    ...
    const ResidentKey key = residentVoxelBlockKey(id);
    auto it = blocks_.find(key);
    if (it == blocks_.end() || expected.source_ == nullptr
        || expected.pin_ == nullptr
        || it->second.voxelSource != expected.source_
        || it->second.pin != expected.pin_) {
        return false;
    }
    residentBytes_ -= it->second.bytes;
    lru_.erase(it->second.lruIt);
