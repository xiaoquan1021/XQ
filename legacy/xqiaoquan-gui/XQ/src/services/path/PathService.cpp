...
// Builds the semantic replace command for an edited control-point set. Validates
                                                ...
                                                double spacing)
{
    ...
    std::unique_ptr<XQCommand> command(
        new SemanticReplacePayloadCommand(
            scene, node.id(), XQDomainType::Path, payload, "Edit path"));
                                                   ...
                                                   double spacing)
{
    ...
    XQDataNode node(newPathId, XQDomainType::Path, name, payload);
    const XQDataNode* sourceImage = scene->find(sourceImageNodeId);
    if (sourceImage != nullptr && sourceImage->hasScaleSlot()) {
        node.setScaleSlot(sourceImage->scaleSlot().value());
    }
    std::unique_ptr<XQCommand> command(
                                                     ...
                                                     double spacing)
{
    ...
    std::unique_ptr<XQCommand> command(
        new SemanticReplacePayloadCommand(
            scene, node.id(), XQDomainType::Path, new_payload, "Resample path"));
    return success(std::move(command));
}
