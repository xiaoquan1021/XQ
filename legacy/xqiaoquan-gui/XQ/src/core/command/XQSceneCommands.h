...
// Semantic content edit: replaces the payload, advances contentRevision once,
// and invalidates every downstream node. Full stale snapshots make undo/redo
// exact even when unrelated nodes were already stale before the edit.
class SemanticReplacePayloadCommand : public XQCommand {
public:
    SemanticReplacePayloadCommand(
        ...
        XQDomainType domain,
        std::shared_ptr<XQPayload> payload,
        std::string label = "Semantic replace payload");
...
private:
    ...
    NodeId id_;
    XQDomainType new_domain_;
    std::shared_ptr<XQPayload> new_payload_snapshot_;
    XQDomainType old_domain_;
    std::shared_ptr<XQPayload> old_payload_snapshot_;
    ContentRevision old_revision_;
    ContentRevision new_revision_;
    XQScene::StaleSnapshot stale_before_;
    XQScene::StaleSnapshot stale_after_;
    bool captured_;
    std::string label_;
};
...
private:
    ...
    XQScene::StaleSnapshot removed_stale_snapshot_;
    std::string label_;
};
