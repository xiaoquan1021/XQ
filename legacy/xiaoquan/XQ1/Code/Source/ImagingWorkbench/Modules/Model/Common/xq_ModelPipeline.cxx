...
auto serializeFaceInfo(const xq_VascularGeometry* geometry) -> std::string
{
    if (!geometry)
        return {};
    ...
    bool first = true;
    for (const auto& face : geometry->GetAllFaceInfos())
    {
        if (!first)
            oss << ';';
        first = false;
    ...
    const xq_CreateModelRequest& request)
{
    ...
    bool usedProfileGroups = false;
    bool usedLegacyContourGroups = false;
    ...
    auto acceptContourGroup = [&](const std::string& nodeName) -> bool
    {
        if (request.sourceContourGroupNames.empty())
            return true;
        return std::find(request.sourceContourGroupNames.begin(),
                         request.sourceContourGroupNames.end(),
                         nodeName) != request.sourceContourGroupNames.end();
    };

    for (const auto& profileNode : profileNodes)
    {
        if (!acceptContourGroup(profileNode->GetName()))
            continue;
    ...
    for (auto it = contourNodes->Begin(); it != contourNodes->End(); ++it)
    {
        ...
        if (!contourGroup)
            continue;
        if (!acceptContourGroup(it->Value()->GetName()))
            continue;
        ...
        if (!pathName.empty() && canonicalPathNames.count(pathName) != 0)
        {
            result.diagnostics.push_back(makeWarning(
                "Legacy ContourGroup '" + it->Value()->GetName() +
                "' on path '" + pathName +
                "' was skipped because a ProfileGroup for the same path was already used."));
            continue;
        }
        ...
        usedLegacyContourGroups = true;
        if (!pathName.empty())
            sourcePaths.push_back(pathName);
    }

    if (usedProfileGroups && usedLegacyContourGroups)
    {
        result.diagnostics.push_back(makeWarning(
            "Model build mixed ProfileGroup and legacy ContourGroup sources. "
            "ProfileGroup sources were preferred when the same path already had a ProfileGroup."));
    }
    ...
    if (request.engine == "occt" && std::string(modeler->Name()) != "occt")
    {
        modelNode->SetBoolProperty("xq.model.algorithm.fallback", true);
        modelNode->SetStringProperty("xq.model.requested_engine", "occt");
        modelNode->SetStringProperty("xq.model.actual_engine", std::string(modeler->Name()).c_str());
    }
    ...
    if (modelElement)
    {
        const auto capInfo = serializeFaceInfo(modelElement);
        if (!capInfo.empty())
            modelNode->SetStringProperty("xq.model.cap_info", capInfo.c_str());
        modelNode->SetStringProperty(
            "xq.model.loft.parameters",
            ("sampling=" + std::to_string(request.numSampling) +
             ";engine=" + request.engine +
             ";blendRadius=" + std::to_string(request.blendRadius)).c_str());
        if (!request.pathFilter.empty())
            modelNode->SetStringProperty("xq.model.trim.path", request.pathFilter.c_str());
        ...
        modelNode->SetIntProperty(
            "xq.model.face_count",
            std::max(qa.faceCount, modelElement->GetFaceNumber()));
    ...
    const auto sourceContourNodes = xq::pipeline::ResolveUpstreamNodes(
        dataStorage,
        modelNode.GetPointer(),
        xq::pipeline::kSourceContourGroupsProperty,
        xq::pipeline::Stage::ContourGroup);
    if (!sourceContourNodes.empty())
        ctxNode = sourceContourNodes.front();
