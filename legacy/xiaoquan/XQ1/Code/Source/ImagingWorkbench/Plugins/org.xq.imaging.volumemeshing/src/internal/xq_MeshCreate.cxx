...
void xq_GridCreate::SelectModel(const QString& modelName)
{
  if (!m_Ui || !m_Ui->comboModel || modelName.isEmpty())
    return;

  const int index = m_Ui->comboModel->findText(modelName);
  if (index >= 0)
    m_Ui->comboModel->setCurrentIndex(index);
}
