...
- **app 层选中渲染策略(GUI 默认,07-03 B5)**:库层 `LodOptions` 默认仍关(上一条不变,`test_scene_renderer` 等价性依赖它)。策略在 app 层:`XQMainWindow` 的选中渲染统一传 `kDefaultLodOptions{enabled=true, interactive=true, budgetTriangles=2'000'000}`——`addSurface`(payload 持内存几何两处)传该常量,`addSurfaceProgressive`(仅 assetId 走 resolver 两处)走 `spec.lod` 同配,`addVolumeMeshProgressive` 不动。默认开是 GUI 的呈现选择,不是改库层缺省;headless 测试与库层 API 仍以「默认关」为基线。
