...
int test_same_major_higher_minor_loads()
{
    ...
    if (!result.diagnostics.empty()) {
        return fail("same-major higher-minor load has no diagnostics", __LINE__);
    }
...
int test_v1_legacy_without_provenance_migrates()
{
    ...
    if (!result.diagnostics.empty()) {
        return fail("legacy v1 migration has no diagnostics", __LINE__);
    }
    if (result.project.state() != xq::XQProject::LifecycleState::Open) {
...
int test_current_v11_roundtrip_preserves_node_ids()
{
    ...
    if (saved_text.find("XQ_NATIVE_PROJECT schemaVersion 1.1\n") == std::string::npos) {
        std::filesystem::remove(path);
        return fail("writer emits schemaVersion 1.1", __LINE__);
    }
    if (saved_text.find("\nprovenance\n") == std::string::npos) {
        std::filesystem::remove(path);
