...
int main()
{
    ...
    if (project.scene().mark_source_changed(xq::NodeId(1003)) != 1) {
        return fail("mark changed path stales contour group", __LINE__);
    }
    if (project.scene().mark_source_changed(xq::NodeId(1004)) != 1) {
        return fail("mark changed contour group stales mesh", __LINE__);
    }
