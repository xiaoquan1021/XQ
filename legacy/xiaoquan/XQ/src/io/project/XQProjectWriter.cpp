...
const char* const kSchemaVersion = "1.2";
const char* const kWriterVersion = "XQ-M9-001";
const char* const kMinimumReaderVersion = "1.2";
...
{
    return coordinateSystem == ImageCoordinateSystem::RAS ? "RAS" : "LPS";
}
...
void write_asset_record(std::ostream& output, const AssetRecord& record,
                        const std::shared_ptr<XQPayload>& payload)
{
    output << "asset " << record.id.serialize()
           << " category " << asset_category_text(record.category)
           << " kind " << asset_kind_text(record.kind)
           << " name ";
    ...
    write_payload_block(output, payload);
...
// / "relations 0" / "endAssets" so the section is always present in a 1.2 doc
// (so loading it back emits no "no asset data" diagnostic).
void write_assets_section(std::ostream& output, const XQProject& project)
{
    ...
    project.scene().visit_nodes(
        [&payload_by_asset](const XQDataNode& node) {
            if (node.hasAssetId() && node.payload()) {
                payload_by_asset[node.assetId()] = node.payload();
            }
        });

    output << "assets " << registry.assetCount() << "\n";
    registry.visit_assets(
        [&output, &payload_by_asset](const AssetRecord& record) {
            ...
            write_asset_record(output, record, payload);
        });
...
bool asset_kind_for_payload(const std::shared_ptr<XQPayload>& payload, AssetKind* out)
{
    ...
    case XQDomainType::Image:
    case XQDomainType::ContourGroup:
        // XQSourcePayload-backed nodes (image/contour/SV mask) register as Image.
        *out = AssetKind::Image;
        return true;
    case XQDomainType::Unknown:
        return false;
...
// publish its large arrays as blobs, bind the node via setAssetId. A node that
// already has an asset id is left untouched (its asset was set up by a caller).
                           ...
                           std::vector<std::string>* derivedRelPaths)
{
    ...
    for (std::vector<NodeId>::const_iterator it = to_derive.begin(); it != to_derive.end(); ++it) {
        ...
        if (!asset_kind_for_payload(node->payload(), &kind)) {
            continue; // Unknown domain: not an entity we persist.
        }
                                              ...
                                              const std::string& projectFilePath)
{
    ...
    const std::string tmpPath = projectFilePath + ".tmp";
    {
        ...
        output << "endScene\n";
        write_assets_section(output, work);
        output << "provenance\n";
        output << "records 1\n";
