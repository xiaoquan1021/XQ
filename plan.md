...
## Completed Phase: Boundary Layers Meshing Infrastructure Action

1. Replace the `boundary-layers` Meshing unsupported guard with a native
   Infrastructure action that uses the existing mesh pipeline fallback.
   - Resolve the selected model node like `generate-volume-mesh`.
   - Read `layer-count` and `growth-rate` from `WorkflowOperationService`.
   - Call `xq_MeshPipelineService::CreateVolumeMesh` with boundary-layer
     request fields populated.
   - Preserve the pipeline's explicit fallback metadata and warning that true
     boundary layers are recorded but not generated by the VTK fallback.
2. Keep `generate-surface-mesh` unsupported until it receives its own handler.
3. Add RED C++ tests before implementation:
   - `boundary-layers` with a real model should no longer return the
     unsupported-operation diagnostic.
   - A successful run should register a mesh result, bind `xq_MitkGrid`, record
     boundary-layer layer/growth metadata, select the result, and refresh
     rendering.
4. Run:
   - `scripts\build-xq.ps1 configure -ExternalsRoot ..\Externals`
   - `scripts\build-xq.ps1 build -ExternalsRoot ..\Externals`
   - targeted `ctest` for `test_monolith_meshing_workflow_action_handler`
   - all XQ `tests\*.ps1`
   - all Externals `tests\*.ps1`
   - `ctest --test-dir .\build\windows-msvc-release --output-on-failure --timeout 120`
   - `git diff --check`
...
## Completed Phase: Loft Profiles Segmentation Infrastructure Action

1. Replace the `loft-profiles` 2D Segmentation unsupported guard with a native
   Infrastructure action.
   - Resolve the selected segmentation node from `DataNodeRegistryService` or
     the active node.
   - Accept canonical `xq_ProfileGroup` sources directly.
   - Accept legacy `xq_ContourGroup` sources by converting through
     `xq_ContourGroupMigration`.
   - Generate and store a loft surface cache with
     `xq_SegmentationUtils::LoftProfileGroup`.
   - Commit a generated Segmentation catalog/hierarchy entry, select it, and
     refresh rendering on success.
2. Keep the action honest about scope.
   - Record whether the source was already canonical or migrated from legacy
     contours.
   - Do not claim Model-stage solid generation; this action only finalizes a
     profile group loft cache for downstream modeling.
3. Add RED C++ tests before implementation:
   - `loft-profiles` with a real contour group should no longer return the
     unsupported-operation diagnostic.
   - A successful run should register a Segmentation result, bind a
     `xq_ProfileGroup`, attach non-empty loft surface geometry, record migration
     metadata, select the result, and refresh rendering.
   - Production composition and the 2D Segmentation page should validate
     `loft-profiles` through the Infrastructure handler.
4. Run:
   - `scripts\build-xq.ps1 configure -ExternalsRoot ..\Externals`
   - `scripts\build-xq.ps1 build -ExternalsRoot ..\Externals`
   - targeted `ctest` for segmentation handler/page/composition tests
...
## Completed Phase: Windows V1 Solver Execution Scope Decision

1. Resolve the ROM/MultiPhysics solver blocker by explicitly deferring solver
   execution from Windows v1.
   - Do not use SimVascular code, solver binaries, schema, or build
     dependencies.
   - Do not add a ROM, 1D/0D, coupled, or FSI solver backend in this phase.
   - Windows v1 keeps ROM and MultiPhysics as pre-solve configuration plus
     review of already-present result data.
2. Hide solver-run operations from default monolith workflow operation lists.
   - `rom-simulation` exposes `build-1d-network` and
     `calibrate-boundary-conditions`.
   - `multiphysics` exposes `configure-coupling` and
     `review-coupled-results`.
   - `run-rom-solver` and `run-coupled-solve` must not be user-selectable from
     the default operation pages.
3. Keep defensive unsupported-operation guards.
   - Direct or stale state selection of `run-rom-solver` must fail without
     creating solver output.
   - Direct or stale state selection of `run-coupled-solve` must fail without
     creating coupled result data.
   - Continue using honest metadata such as `configured`, `calibrated`,
     `ready`, and `not_solver_run`.
4. Future resume criteria:
