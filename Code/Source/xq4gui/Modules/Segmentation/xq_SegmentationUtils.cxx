...
{
    return vectorLength(v) > kEpsilon ? xq_Math3::UnitVector(v) : fallback;
}
    ...
    mitk::Vector3D& normal)
{
    ...
    for (int i = 0; i < profile->GetAnchorPointCount(); ++i)
    {
        ...
        {
            axis0 = xq_Math3::UnitVector(delta);
            break;
        }
    }
    ...
    normal = normalizedOrFallback(
        ...
        fallbackNormal);
    axis1 = normalizedOrFallback(crossProduct(normal, axis0), xq_Math3::ComputeOrthogonalVector(axis0));
    axis0 = normalizedOrFallback(crossProduct(axis1, normal), fallbackAxis0);
}
    ...
    const mitk::PlaneGeometry* referencePlane)
{
    ...
    auto right = normalizedOrFallback(frame.rotation, xq_Math3::ComputeOrthogonalVector(normal));
    auto bottom = xq_Math3::CrossProduct3D(normal, right);
    if (vectorLength(bottom) <= kEpsilon)
    {
        right = xq_Math3::ComputeOrthogonalVector(normal);
        bottom = xq_Math3::CrossProduct3D(normal, right);
    }
    bottom = xq_Math3::UnitVector(bottom);
    right = xq_Math3::UnitVector(xq_Math3::CrossProduct3D(bottom, normal));
    ...
    double secondarySize)
{
    ...
    axis1 = normalizedOrFallback(axis1, xq_Math3::ComputeOrthogonalVector(axis0));
