...
{
    // TODO(xq-levelset): integrate itkGeodesicActiveContourLevelSetImageFilter
    // with seed-based initialization. Until then we fall back to threshold
    // around the seed's local intensity.
    xq_ThresholdSegmentation th;
    ...
    if (slice.slice)
    {
        ...
        fallback.threshold = seedVal * 0.5; // crude but harmless fallback
    }
    auto r = th.Extract(slice, fallback);
    if (r.ok)
        r.diagnostic = "LevelSet algorithm not yet linked; returned threshold fallback.";
    return r;
}
