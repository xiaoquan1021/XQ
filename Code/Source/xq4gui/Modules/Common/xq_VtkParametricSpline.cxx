...
xq_VtkParametricSpline::GetSplineFramePoints()
{
    ...
    for (int i = 0; i < m_Resolution; ++i)
    {
        ...
        sp.tangent = xq_Math3::UnitVector(tangent);
        ...
        if (firstFrame)
        {
            frameXAxis = xq_Math3::ComputeOrthogonalVector(sp.tangent);
            firstFrame = false;
        }
        else
        {
            ...
            double proj = xq_Math3::DotProduct3D(prevXAxis, sp.tangent);
            mitk::Vector3D n;
            n[0] = prevXAxis[0] - proj * sp.tangent[0];
            n[1] = prevXAxis[1] - proj * sp.tangent[1];
            n[2] = prevXAxis[2] - proj * sp.tangent[2];
            double nLen = std::sqrt(xq_Math3::DotProduct3D(n, n));
            if (nLen > 1e-12)
            {
                frameXAxis = xq_Math3::UnitVector(n);
            }
            else
            {
                frameXAxis = xq_Math3::ComputeOrthogonalVector(sp.tangent);
        ...
        const auto frameBinormal = xq_Math3::CrossProduct3D(sp.tangent, frameXAxis);
        sp.rotation = frameXAxis;
        sp.normal = xq_Math3::UnitVector(frameBinormal);
