  ...
  const QList<mitk::DataNode::Pointer>& nodes)
{
  ...
  for (const auto& node : nodes)
  {
    if (node.IsNotNull())
    {
      if (GetProfileGroup(node) != nullptr || GetContourGroup(node) != nullptr)
      {
        ...
        {
          const auto pathName = QString::fromStdString(profileGroup->GetAttribute("path_name"));
  ...
  bool hasGroup = m_CurrentContourGroupNode.IsNotNull();
  m_Ui->btnDeleteGroup->setEnabled(hasGroup);
  m_Ui->btnLoft->setEnabled(hasGroup);
  m_Ui->btnAddContour->setEnabled(hasGroup);
  m_Ui->btnDeleteContour->setEnabled(hasGroup);
  UpdatePathReslice();
}
...
mitk::Image::Pointer xq_LumenContouringView::FindSelectedImage()
{
  ...
  {
    if (it->Value()->IsVisible(nullptr))
    {
      auto* img = dynamic_cast<mitk::Image*>(it->Value()->GetData());
      if (img) return img;
    }
  }
  return nullptr;
...
void xq_LumenContouringView::PerformAutoSegmentation()
{
  ...
  if (image.IsNull())
  {
    QMessageBox::warning(nullptr, "Auto Segmentation",
                         "No visible image found.  Load and display an image first.");
    return;
  }
