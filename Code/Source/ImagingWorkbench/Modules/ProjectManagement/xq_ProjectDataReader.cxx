...
mitk::DataNode::Pointer LoadXqROMJob(const std::string& fullPath)
{
    auto node = mitk::DataNode::New();
    auto result = xq_MitkROMJobIO::Read(node, fullPath);
    return result.ok ? node : nullptr;
}

mitk::DataNode::Pointer LoadXqMultiPhysicsJob(const std::string& fullPath)
{
    auto node = mitk::DataNode::New();
    auto result = xq_MitkMultiPhysicsJobIO::Read(node, fullPath);
    return result.ok ? node : nullptr;
}

xq_MitkSeg3D::Seg3DMethod ParseSeg3DMethod(const std::string& method)
{
    if (method == "Region Growing" || method == "region_growing")
        return xq_MitkSeg3D::Seg3DMethod::REGION_GROWING;
    if (method == "Level Set" || method == "level_set")
        return xq_MitkSeg3D::Seg3DMethod::LEVEL_SET;
    if (method == "Colliding Fronts" || method == "colliding_fronts")
        return xq_MitkSeg3D::Seg3DMethod::COLLIDING_FRONTS;
    return xq_MitkSeg3D::Seg3DMethod::THRESHOLD;
}

mitk::DataNode::Pointer LoadXqSeg3D(const std::string& fullPath)
{
    ...
    const char* method = root->Attribute("method");
    seg3d->SetMethod(ParseSeg3DMethod(method ? method : "Threshold"));

    if (auto* thresholdsElem = root->FirstChildElement("thresholds"))
    {
        seg3d->SetLowerThreshold(thresholdsElem->DoubleAttribute("lower", 0.0));
        seg3d->SetUpperThreshold(thresholdsElem->DoubleAttribute("upper", 1000.0));
    }

    if (auto* seedsElem = root->FirstChildElement("seed_points"))
    {
        for (auto* pointElem = seedsElem->FirstChildElement("point");
             pointElem;
             pointElem = pointElem->NextSiblingElement("point"))
        {
            mitk::Point3D point;
            point[0] = pointElem->DoubleAttribute("x", 0.0);
            point[1] = pointElem->DoubleAttribute("y", 0.0);
            point[2] = pointElem->DoubleAttribute("z", 0.0);
            seg3d->AddSeedPoint(point);
        }
    }

    if (auto* vtpElem = root->FirstChildElement("vtp_file"))
    {
        const char* relPath = vtpElem->Attribute("path");
        if (relPath && relPath[0] != '\0')
        {
            const auto slashPos = fullPath.find_last_of("/\\");
            const std::string dir = slashPos == std::string::npos
                ? std::string()
                : fullPath.substr(0, slashPos + 1);
            const std::string vtpPath = dir + relPath;
    ...
    const std::string& fullPath)
{
    ...
    if (EndsWith(fullPath, ".xqrom.xml"))
    {
        auto node = LoadXqROMJob(fullPath);
        if (node)
        {
            result.nodes.push_back(node);
            result.ok = true;
            ...
            result.diagnostics.push_back("Failed to load xqrom: " + fullPath);
        return result;
    }

    if (EndsWith(fullPath, ".xqmp.xml"))
    {
        auto node = LoadXqMultiPhysicsJob(fullPath);
        if (node)
        {
            result.nodes.push_back(node);
            result.ok = true;
            ...
            result.diagnostics.push_back("Failed to load xqmp: " + fullPath);
        return result;
    }

    if (EndsWith(fullPath, ".xqseg3d"))
    {
        auto node = LoadXqSeg3D(fullPath);
        if (node)
        {
            result.nodes.push_back(node);
            result.ok = true;
            ...
            result.diagnostics.push_back("Failed to load xqseg3d: " + fullPath);
        return result;
    }
