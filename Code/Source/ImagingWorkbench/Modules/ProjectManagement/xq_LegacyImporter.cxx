...
static bool HasFinalSuffix(const std::string& value)
{
    return value.size() > 6 && value.rfind("_final") == value.size() - 6;
}

static std::string NormalizeImportedPathName(
    const std::string& value,
    const std::string* canonicalName = nullptr)
{
    if (!HasFinalSuffix(value))
        return value;

    const std::string stripped = value.substr(0, value.size() - 6);
    if (canonicalName && *canonicalName == stripped)
        return stripped;
    return value;
}
...
mitk::DataNode::Pointer xq_LegacyImporter::ParsePathFile(const std::string& filePath)
{
    ...
    if (auto* pathPointsElem = pathElement->FirstChildElement("path_points"))
    {
             ...
             ppElem = ppElem->NextSiblingElement("path_point"))
        {
            ...
            vertex.tangent = xq_SpatialMath::UnitVector(ParseVector3D(tangentElem));
            vertex.rotation = xq_SpatialMath::UnitVector(ParseVector3D(rotationElem));
            vertex.normal = xq_SpatialMath::UnitVector(
                xq_SpatialMath::CrossProduct3D(vertex.tangent, vertex.rotation));
    ...
    const std::string& filePath)
{
    ...
    else if (pathId != 0)
    {
        const auto idIt = m_PathIdToName.find(pathId);
        if (idIt != m_PathIdToName.end())
            pathName = NormalizeImportedPathName(pathName, &idIt->second);
    }
    if (pathName.empty())
        pathName = GetFileNameOnly(filePath);
         ...
         contourElem = contourElem->NextSiblingElement("contour"))
    {
        ...
        if (auto* ppElem = contourElem->FirstChildElement("path_point"))
        {
            ...
            frame.tangent = xq_SpatialMath::UnitVector(ParseVector3D(ppElem->FirstChildElement("tangent")));
            frame.rotation = xq_SpatialMath::UnitVector(ParseVector3D(ppElem->FirstChildElement("rotation")));
