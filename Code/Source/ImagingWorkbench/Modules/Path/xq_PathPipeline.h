...
struct XQMODULEPATH_EXPORT xq_PathExtractRequest
{
    std::string pathName;
    std::string centerlineNodeName; // REQUIRED: existing centerline node by name
    std::string imageNodeName;      // Optional: restored from centerline metadata if empty
    int sampleCount = 200;
    bool smoothCurve = true;
};

struct XQMODULEPATH_EXPORT xq_PathExtractResult
    : public xq::pipeline::OperationStatus
{
    mitk::DataNode::Pointer node;
    xq_VesselCenterline* path = nullptr;
};
...
public:
    ...
    // Convert an existing centerline representation into a normal XQ Path
    // node. This does not compute centerlines; it only extracts paths from
    // real, already-present centerline geometry.
    static xq_PathExtractResult ExtractPathFromCenterline(
        mitk::DataStorage* dataStorage,
