...
class XQ_QT_SEGMENTATION_EXPORT xq_LumenContouringView
...
private:
  ...
  void EnableContourControls(bool enabled);
  void PopulatePathComboBox();
  void UpdatePathReslice();
  mitk::Image::Pointer FindSelectedImage();
  mitk::DataNode::Pointer FindSelectedImageNode();
  void PersistContourToolMetadata();
