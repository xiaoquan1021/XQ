...
void xq_ProfileGroupCreate::SelectPath(const QString& pathName)
{
  if (!m_Ui || !m_Ui->comboPath || pathName.isEmpty())
    return;

  const int index = m_Ui->comboPath->findText(pathName);
  if (index >= 0)
    m_Ui->comboPath->setCurrentIndex(index);
}
