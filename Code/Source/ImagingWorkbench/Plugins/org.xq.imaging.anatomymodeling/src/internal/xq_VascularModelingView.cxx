...
bool IsContourGroupNode(const mitk::DataNode::Pointer& node)
{
  if (node.IsNull())
    return false;
...
void xq_VascularModelingView::CreateQtPartControl(QWidget* parent)
{
  ...
  connect(m_Ui->tableFaces, &QTableWidget::currentCellChanged,
          this, [this](int currentRow, int /*currentColumn*/, int /*previousRow*/, int /*previousColumn*/) {
            if (m_CurrentModelNode.IsNotNull() && currentRow >= 0)
              m_CurrentModelNode->SetIntProperty("xq.model.current_face_index", currentRow);
          });
...
void xq_VascularModelingView::BindCurrentDataManagerSelection()
{
...
void xq_VascularModelingView::PopulateFaceTable(mitk::DataNode::Pointer modelNode)
{
  ...
  if (geometry && geometry->GetFaceNumber() > 0)
  {
    ...
    if (modelNode->GetIntProperty("xq.model.current_face_index", currentFaceIndex) &&
        currentFaceIndex >= 0 && currentFaceIndex < m_Ui->tableFaces->rowCount())
    {
      m_Ui->tableFaces->setCurrentCell(currentFaceIndex, 0);
    }
    return;
  }
...
void xq_VascularModelingView::ClearModelState()
{
  m_CurrentModelNode = nullptr;
  ClearFaceTable();
  EnableModelControls(false);
...
void xq_VascularModelingView::EnableModelControls(bool enabled)
{
  if (!m_Ui)
    return;

  m_Ui->btnDeleteModel->setEnabled(enabled);
  m_Ui->btnExportModel->setEnabled(enabled);
  m_Ui->btnModelStats->setEnabled(enabled);
  m_Ui->btnExtractCenterlines->setEnabled(enabled);
  m_Ui->btnBooleanUnion->setEnabled(enabled);
  m_Ui->btnBooleanSubtract->setEnabled(enabled);
  m_Ui->btnApplyFillet->setEnabled(enabled);
  m_Ui->btnChangeFaceColor->setEnabled(enabled);
  m_Ui->btnToggleFaceVisibility->setEnabled(enabled);
  m_Ui->btnDecimate->setEnabled(enabled);
  m_Ui->btnComputeNormals->setEnabled(enabled);
  m_Ui->btnCleanSurface->setEnabled(enabled);
  m_Ui->btnFillHoles->setEnabled(enabled);
}

void xq_VascularModelingView::UpdateContextStatus(const QString& status)
{
  if (!m_ContextLabel)
    return;
  ...
  const QString modelName = m_CurrentModelNode.IsNotNull()
    ...
    : QStringLiteral("<none>");
  QString source = QStringLiteral("<unresolved>");
  if (m_CurrentModelNode.IsNotNull())
  {
    std::string value;
    if (m_CurrentModelNode->GetStringProperty(xq::pipeline::kSourceContourGroupProperty, value) &&
        !value.empty())
      source = QString::fromStdString(value);
    else if (m_CurrentModelNode->GetStringProperty(xq::pipeline::kSourceSegmentationProperty, value) &&
             !value.empty())
      source = QString::fromStdString(value);
  }
  const int faceRows = (m_Ui && m_Ui->tableFaces) ? m_Ui->tableFaces->rowCount() : 0;
  QString qaSummary = QStringLiteral("not evaluated");
  if (m_CurrentModelNode.IsNotNull())
  {
    bool qaOk = false;
    if (m_CurrentModelNode->GetBoolProperty("xq.model.qa.ok", qaOk))
    {
      int boundaryEdges = -1;
      ...
      m_CurrentModelNode->GetIntProperty("xq.model.qa.boundary_edges", boundaryEdges);
      m_CurrentModelNode->GetIntProperty("xq.model.qa.non_manifold_edges", nonManifoldEdges);
      m_CurrentModelNode->GetIntProperty("xq.model.qa.connected_components", connectedComponents);
      m_CurrentModelNode->GetIntProperty("xq.model.face_count", faceCount);
      m_CurrentModelNode->GetBoolProperty("xq.model.qa.has_face_ids", hasFaceIds);
      qaSummary = QString("ok=%1, boundaryEdges=%2, nonManifoldEdges=%3, components=%4, faceIds=%5, faces=%6")
        .arg(qaOk ? "yes" : "no")
        .arg(boundaryEdges)
        .arg(nonManifoldEdges)
        .arg(connectedComponents)
        .arg(hasFaceIds ? "yes" : "no")
        .arg(faceCount);
    }
    else
    {
      ...
      qaSummary = polyData
        ? QString("not stored, points=%1, cells=%2")
            .arg(polyData->GetNumberOfPoints())
            .arg(polyData->GetNumberOfCells())
        : QStringLiteral("invalid: no surface geometry");
    }
  }
  ...
  m_ContextLabel->setText(
    QString("Current anatomy model: %1\nUpstream segmentation/contours: %2\nFace/cap rows: %3\nQA: %4\nStatus: %5\nNext: verify faces/caps before volume meshing.")
      .arg(modelName, source)
      .arg(faceRows)
      .arg(qaSummary, resolvedStatus));
}
...
void xq_VascularModelingView::CreateModel()
{
  ...
  {
    std::vector<std::string> preselectedGroups;
    if (GetSite() && GetSite()->GetWorkbenchWindow().IsNotNull())
    {
      ...
      if (selectionService)
      {
        ...
        if (nodeSelection.IsNotNull())
        {
          ...
          for (const auto& node : selectedNodes)
          {
            if (IsContourGroupNode(node))
              preselectedGroups.push_back(node->GetName());
          }
        }
      }
    }
    dialog.SelectSegmentationGroups(preselectedGroups);
  }
  ...
  if (dialog.exec() == QDialog::Accepted)
  {
    ...
    request.modelName = modelName.toStdString();
    request.modelType = modelType.toStdString();
    request.numSampling = numSampling;
    request.sourceContourGroupNames = dialog.GetSelectedSegmentationGroups();

    const auto createResult = xq_ModelPipelineService::CreateModel(
      GetDataStorage(), request);
...
void xq_VascularModelingView::ExportModel()
{
  ...
  if (auto* model = dynamic_cast<xq_Model*>(m_CurrentModelNode->GetData()))
  {
    if (auto* element = model->GetModelElement(0))
      element->EmbedFaceInfoToPolyData(polyData);
  }
...
void xq_VascularModelingView::BooleanUnion()
{
  ...
  if (m_CurrentModelNode->GetStringProperty(
        xq::pipeline::kSourceContourGroupsProperty, sourceContourGroups) &&
      !sourceContourGroups.empty())
    xq::pipeline::SetStringProperty(
      resultNode, xq::pipeline::kSourceContourGroupsProperty, sourceContourGroups);
  std::string modelType;
  if (m_CurrentModelNode->GetStringProperty("xq.model.type", modelType) && !modelType.empty())
    resultNode->SetStringProperty("xq.model.type", modelType.c_str());
  resultNode->SetStringProperty(xq::pipeline::kAlgorithmProperty, "boolean_union");
...
void xq_VascularModelingView::BooleanSubtract()
{
  ...
  if (m_CurrentModelNode->GetStringProperty("xq.model.type", modelType) && !modelType.empty())
    resultNode->SetStringProperty("xq.model.type", modelType.c_str());
  resultNode->SetStringProperty(xq::pipeline::kAlgorithmProperty, "boolean_subtract");
  resultNode->SetStringProperty("xq.status", "modified");
