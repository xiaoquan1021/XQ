...
private:
  ...
  void OnDataNotesDoubleClicked(int row, int column);
  void OnDataTreeDoubleClicked(const QModelIndex& index);
  void OpenToolForNode(mitk::DataNode* node);
  mitk::DataNode* ResolveDataNotesTargetNode(int row);
  void SelectNodeInTree(mitk::DataNode* node);
