...
void xq_VesselPlanningView::CreateQtPartControl(QWidget* parent)
{
  ...
  m_Ui->pathLayout->insertWidget(0, m_ContextLabel);

  auto defaultPlanner = CreateDefaultPathPlanner();
  auto* algorithmStatusLabel = new QLabel(parent);
  if (defaultPlanner && defaultPlanner->Name() == std::string_view("dijkstra"))
    algorithmStatusLabel->setText("Automatic path planner: Dijkstra (XQ-native)");
  else
    algorithmStatusLabel->setText("Automatic path planner: unavailable");
  algorithmStatusLabel->setWordWrap(true);
  m_Ui->pathLayout->insertWidget(1, algorithmStatusLabel);
  ...
  UpdateContextStatus("Missing path");
  ...
  connect(m_ResliceSizeSpin, QOverload<double>::of(&QDoubleSpinBox::valueChanged),
          this, [this](double value) {
            if (m_CurrentPathNode.IsNotNull())
              m_CurrentPathNode->SetDoubleProperty("xq.path.reslice_size", value);
            UpdatePathReslice();
          });
...
void xq_VesselPlanningView::Activated()
{
  BindCurrentDataManagerSelection();
}

void xq_VesselPlanningView::Deactivated()
{
  DeactivateToolState();
}

void xq_VesselPlanningView::Visible()
{
}

void xq_VesselPlanningView::Hidden()
{
  DeactivateToolState();
}

void xq_VesselPlanningView::RenderWindowPartActivated(mitk::IRenderWindowPart* /*renderWindowPart*/)
{
  UpdatePathReslice();
}

void xq_VesselPlanningView::RenderWindowPartDeactivated(mitk::IRenderWindowPart* /*renderWindowPart*/)
{
  DeactivateToolState();
}

void xq_VesselPlanningView::DeactivateToolState()
{
...
void xq_VesselPlanningView::BindCurrentDataManagerSelection()
{
    ...
    const QList<mitk::DataNode::Pointer>& nodes)
{
  for (const auto& node : nodes)
  {
    if (node.IsNotNull())
    {
      ...
      {
        if (m_CurrentPathNode == node)
        {
          RestorePathToolMetadata();
          EnablePathControls(true);
          UpdatePointTable();
          UpdatePathReslice();
...
void xq_VesselPlanningView::AddPath()
{
  ...
  if (dialog.exec() == QDialog::Accepted)
  {
    ...
    node->SetStringProperty("xq.path.method", "manual");
    node->SetIntProperty("xq.path.subdivision_number", dialog.GetSubdivisionNumber());
    node->SetIntProperty("xq.path.calculation_number", dialog.GetCalculationNumber());
    node->SetFloatProperty("point size", 1.0f);
    ...
    const QItemSelection& /*deselected*/)
{
  ...
  if (m_CurrentPathNode.IsNotNull() && hasSelection)
    m_CurrentPathNode->SetIntProperty(
      "xq.path.current_point_index", selected.indexes().first().row());
}
...
void xq_VesselPlanningView::UpdatePathTable()
{
  ...
  const QSignalBlocker viewBlocker(m_PathTableView);
  m_PathTableView->setUpdatesEnabled(false);
  ...
  m_PathTableView->setUpdatesEnabled(true);
}
...
void xq_VesselPlanningView::UpdatePointTable()
{
  ...
  const QSignalBlocker modelBlocker(model);
  const QSignalBlocker viewBlocker(m_PointTableView);
  m_PointTableView->setUpdatesEnabled(false);
  model->removeRows(0, model->rowCount());

  if (m_CurrentPathNode.IsNull())
  {
    m_PointTableView->setUpdatesEnabled(true);
    return;
  }
  ...
  if (m_CurrentPathNode->GetIntProperty("xq.path.current_point_index", currentPointIndex) &&
      currentPointIndex >= 0 &&
      currentPointIndex < model->rowCount() &&
      m_PointTableView->selectionModel())
  {
    m_PointTableView->selectRow(currentPointIndex);
  }
  m_PointTableView->setUpdatesEnabled(true);
}

void xq_VesselPlanningView::ClearPathState()
{
  if (m_PathInteractor.IsNotNull() && m_PathInteractor->GetDataNode())
    m_PathInteractor->GetDataNode()->SetDataInteractor(nullptr);
...
void xq_VesselPlanningView::EnablePathControls(bool enabled)
{
  if (!m_Ui)
    return;

  m_Ui->deletePathButton->setEnabled(enabled);
  m_Ui->addPointButton->setEnabled(enabled);
  m_Ui->deletePointButton->setEnabled(false);
  m_Ui->smartPointButton->setEnabled(false);
  m_Ui->interpolateButton->setEnabled(enabled);
  m_Ui->statsButton->setEnabled(enabled);
  m_Ui->exportButton->setEnabled(enabled);
  m_Ui->btnResamplePath->setEnabled(enabled);
  m_Ui->btnReversePath->setEnabled(enabled);
  m_Ui->btnMergePaths->setEnabled(enabled);
  m_Ui->btnCrossSection->setEnabled(enabled);
  m_Ui->btnSmoothPath->setEnabled(enabled);
}

void xq_VesselPlanningView::UpdateContextStatus(const QString& status)
{
  if (!m_ContextLabel)
    return;

  const QString pathName = m_CurrentPathNode.IsNotNull()
    ? QString::fromStdString(m_CurrentPathNode->GetName())
    : QStringLiteral("<none>");
  QString sourceImage = QStringLiteral("<unresolved>");
  if (m_CurrentPathNode.IsNotNull())
  {
    std::string imageName;
    if (m_CurrentPathNode->GetStringProperty(xq::pipeline::kSourceImageProperty, imageName) &&
        !imageName.empty())
    {
      sourceImage = QString::fromStdString(imageName);
    }
    else
    {
      auto imageNode = FindSelectedImageNode();
      if (imageNode.IsNotNull())
        sourceImage = QString::fromStdString(imageNode->GetName());
    }
  }

  const QString resolvedStatus = status.isEmpty()
    ? (m_CurrentPathNode.IsNotNull() ? QStringLiteral("Ready") : QStringLiteral("Missing path"))
    : status;
  m_ContextLabel->setText(
    QString("Current editable path: %1\nInput image: %2\nStatus: %3\nNext: add/edit path points, then use Lumen Segmentation.")
      .arg(pathName, sourceImage, resolvedStatus));
}
...
void xq_VesselPlanningView::InterpolatePath()
{
  ...
  interpNode->SetFloatProperty("point size", 1.5f);
  ...
  interpNode->SetStringProperty("xq.path.method", "interpolated");
  interpNode->SetIntProperty("xq.path.point_count", static_cast<int>(interpPoints.size()));
...
void xq_VesselPlanningView::ResamplePath()
{
  ...
  m_CurrentPathNode->SetStringProperty(xq::pipeline::kAlgorithmProperty, "resampled");
  m_CurrentPathNode->SetStringProperty("xq.path.method", "resampled");
  m_CurrentPathNode->SetDoubleProperty("xq.path.spacing", spacing);
  m_CurrentPathNode->SetIntProperty("xq.path.point_count", static_cast<int>(resampled.size()));
  centerline->Modified();
  UpdatePointTable();
  PersistPathToolMetadata();
  UpdatePathTable();
...
void xq_VesselPlanningView::ReversePath()
{
  ...
  m_CurrentPathNode->SetStringProperty(xq::pipeline::kAlgorithmProperty, "reversed");
  m_CurrentPathNode->SetStringProperty("xq.path.method", "reversed");
  m_CurrentPathNode->SetIntProperty("xq.path.point_count", numPoints);
  centerline->Modified();
  UpdatePointTable();
  PersistPathToolMetadata();
  mitk::RenderingManager::GetInstance()->RequestUpdateAll();
...
void xq_VesselPlanningView::MergePaths()
{
  ...
  mergedNode->SetStringProperty("xq.path.method", "merged");
  mergedNode->SetIntProperty("xq.path.point_count", static_cast<int>(mergedPoints.size()));
...
void xq_VesselPlanningView::SmoothPath()
{
  ...
  smoothNode->SetFloatProperty("point size", 1.5f);
  ...
  smoothNode->SetStringProperty("xq.path.method", method.toStdString().c_str());
  smoothNode->SetIntProperty("xq.path.smoothing_output_count", numOutput);
  smoothNode->SetIntProperty("xq.path.smoothing_modes", numModes);
  smoothNode->SetIntProperty("xq.path.point_count", static_cast<int>(smoothed.size()));
...
mitk::Image::Pointer xq_VesselPlanningView::FindSelectedImage() const
{
  ...
  return node.IsNotNull() ? dynamic_cast<mitk::Image*>(node->GetData()) : nullptr;
}
...
mitk::DataNode::Pointer xq_VesselPlanningView::FindSelectedImageNode() const
{
  ...
  if (m_CurrentPathNode.IsNotNull() &&
      m_CurrentPathNode->GetStringProperty(
...
void xq_VesselPlanningView::RestorePathToolMetadata()
{
  if (m_CurrentPathNode.IsNull())
    return;

  double spacing = 0.0;
  if (m_Ui && m_Ui->spinResampleSpacing &&
      m_CurrentPathNode->GetDoubleProperty("xq.path.spacing", spacing) &&
      spacing > 0.0)
  {
    m_Ui->spinResampleSpacing->setValue(static_cast<int>(std::round(spacing)));
  }

  double resliceSize = 0.0;
  if (m_ResliceSizeSpin &&
      m_CurrentPathNode->GetDoubleProperty("xq.path.reslice_size", resliceSize) &&
      resliceSize > 0.0)
  {
  ...
  if (m_CurrentPathNode->GetDoubleProperty("xq.path.point_size", pointSize) &&
      pointSize > 0.0)
  {
    m_CurrentPathNode->SetFloatProperty("point size", static_cast<float>(pointSize));
  }
}

void xq_VesselPlanningView::PersistPathToolMetadata()
{
  if (m_CurrentPathNode.IsNull())
    return;
  ...
  {
    m_CurrentPathNode->SetIntProperty(
      "xq.path.point_count", segment->GetAnchorCount());
    m_CurrentPathNode->SetDoubleProperty(
      "xq.path.spacing", segment->GetStepSize());
  }

  if (m_ResliceSizeSpin)
    m_CurrentPathNode->SetDoubleProperty(
      "xq.path.reslice_size", m_ResliceSizeSpin->value());

  float pointSize = 1.0f;
  if (m_CurrentPathNode->GetFloatProperty("point size", pointSize))
    m_CurrentPathNode->SetDoubleProperty("xq.path.point_size", pointSize);

  if (m_PointTableView && m_PointTableView->selectionModel())
  {
    const QModelIndexList selected =
      m_PointTableView->selectionModel()->selectedRows();
    if (!selected.isEmpty())
      m_CurrentPathNode->SetIntProperty(
        "xq.path.current_point_index", selected.first().row());
  }

  m_CurrentPathNode->SetBoolProperty("xq.path.editable", true);
  m_CurrentPathNode->Modified();
}
